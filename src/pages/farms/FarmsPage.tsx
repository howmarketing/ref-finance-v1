...
useEffect(() =>{
    const dataChangeDelayToApply = setTimeout(() => {
        console.log("Loaded data: ",{
            farms, stakedList, rewardList, tokenPriceList, seeds, tokenPriceMap, yourFarms, yourReward, lps
        });
    },5000);
    return () =>{return clearTimeout(dataChangeDelayToApply);}
},[farms, stakedList, rewardList, tokenPriceList, seeds, tokenPriceMap, yourFarms, yourReward, lps]);
  ...
  async function loadFarmInfoList() {
    ...
    const composeFarms = (farms: FarmInfo[]) => {
      ...
      console.log("tempFarms: ",tempFarms);
      return tempFarms;
    };
  ...
  async function getTokenSinglePrice(
    farms: Array<FarmInfo & Record<any, any>>,
    rewardList: Record<string, string>,
    tokenPriceList: Record<string, any>
  ) {
    const rewardTokenList: Record<string, typeof farms[0]["rewardToken"] & Record<any, any>> = {};
    farms.forEach((item) => {
...
function FarmView({
  ...
  function claimReward() {
    ...
    console.log(farmsData.length > 1 ? 'claimRewardBySeed' : 'claimRewardByFarm');
    if (farmsData.length > 1) {
      claimRewardBySeed(data.seed_id)
        .then(() => {
            console.log('claimRewardBySeed executed.');
      ...
      claimRewardByFarm(data.farm_id)
        .then(() => {
            console.log('claimRewardByFarm executed.');
            // window.location.reload();
        })
        .catch((error) => {
          setDisableClaim(false);
  ...
  return (
    ...
    >
      ...
      <ActionModal
        ...
        tokenPriceList={tokenPriceList}
        onSubmit={(amount) => {
            const stakeValues = { lp_token_id:data.lpTokenId, token_id: getMftTokenId(data.lpTokenId), amount };
...
function ActionModal(
  ...
  useEffect(() => {
    console.log("Amount changed for stake token in farm: ",{
        amount,
        farm
    });
  },[amount]);
      ...
      {showTip ? (
        ...
        <div>
          ...
          >
            ...
            {type == 'stake' ? (
              <div className="mt-4" data-name="stake area">
                <div className="flex flex-col items-center justify-center" data-name="stake wrap">
                  <div
                    className="flex items-center justify-center mb-2 cursor-pointer"
                    data-name="stake show calc wrap"
                    onClick={showCalcWrap}
                  >
                    <Calc></Calc>
                    <label className="text-sm text-white ml-3 mr-4  cursor-pointer" data-name="stake calculate roi">
